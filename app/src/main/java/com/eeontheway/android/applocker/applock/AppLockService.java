...
public class AppLockService extends Service {
    ...
    private static final String ACTION_NEW_APP_UNLOCKED = "AppLockService.newAppUnlocked";
    private static final String PARAM_APP_NAME = "packagename";

    private List<BaseAppInfo> appInfoList = new ArrayList<>();
    private List<AppLockConfigInfo> appLockConfigInfoList = new ArrayList<>();

    /**
     * 启动应用锁服务
     */
    public static void startBlockService(Context context) {
        if (ServiceUtils.isServiceRunning(context, AppLockService.class.getName()) == false) {
    ...
    public static void stopBlockService(Context context) {
        if (ServiceUtils.isServiceRunning(context, AppLockService.class.getName()) == true) {
            Intent intent = new Intent(context, AppLockService.class);
    ...
    private void registerAppUnlockReceiver() {
        ...
        appUnlockReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                appLockUnlockedList.add(packageName);
            }
        };
    ...
    private void registerScreenLockReceiver() {
        ...
        screenLockReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                int mode = appLockSettingsManager.getScreenUnlockLockMode();
                switch (mode) {
                    case AppLockSettingsManager.SCREENLOCK_CLEAN_NO_LOCK:
                        // 不需要做任何处理，这样已经锁的和不锁的状态保持不变
                        break;
                    case AppLockSettingsManager.SCREENLOCK_CLEAN_ALL_LOCKED:
                        // 清除所有已经解锁应用的锁定状态，
                        appLockUnlockedList.clear();
                        break;
                    case AppLockSettingsManager.SCREENLOCK_CLEAN_ALL_LOCKED_AFTER_3MIN:
                        // 启动延时操作
    ...
    public void onCreate() {
        ...
        appLockUnlockedList = new AppLockUnlockedList(this);
...
//        configInfo.setPackageName(getPackageName());
    ...
    class WatchThreadRunnable implements Runnable {
        ...
        private AppInfoManager infoManager = new AppInfoManager(AppLockService.this);
        ...
        private boolean isNeedLock () {
            ...
            // 已经解锁过
            if (appLockUnlockedList.contains(currentPackageName)) {
                return false;
            }
            ...
            // 数据库中配置不需要
            synchronized (appLockConfigInfoList) {
                for (AppLockConfigInfo configInfo : appLockConfigInfoList) {
        ...
        public void run() {
            ...
            while (!quit) {
                ...
                if (appLockSettingsManager.isAutoLockOnAppQuit()) {
                    appLockUnlockedList.removeExitedApps();
                }
                ...
                if (currentPackageName != null) {
                    ...
                    if (!currentPackageName.equals(mypackageName)) {
                        appLockUnlockedList.remove(mypackageName);
                    }

                    // 如果启动了一次解锁即解锁全部，且当前应用并不是自己
                    if (appLockSettingsManager.isUnlockAnyUnlockAllEnabled() &&
                            !currentPackageName.equals(mypackageName)) {
                        if (appLockUnlockedList.isEmpty() == false) {
                            // 小睡一会儿，然后再重新检查锁定
                            sleep();
                            continue;
                        }
